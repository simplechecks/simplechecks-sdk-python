...
pip install git+ssh://git@github.com/stainless-sdks/simplechecks-python.git
```

> [!NOTE]
> Once this package is [published to PyPI](https://www.stainless.com/docs/guides/publish), this will become: `pip install simplechecks`
...
```sh
# install from this staging repo
pip install 'simplechecks[aiohttp] @ git+ssh://git@github.com/stainless-sdks/simplechecks-python.git'
```
...
When the library is unable to connect to the API (for example, due to network connection problems or a timeout), a subclass of `simplechecks.APIConnectionError` is raised.

When the API returns a non-success status code (that is, 4xx or 5xx
response), a subclass of `simplechecks.APIStatusError` is raised, containing `status_code` and `response` properties.

All errors inherit from `simplechecks.APIError`.
...
except simplechecks.RateLimitError as e:
    print("A 429 status code was received; we should back off a bit.")
except simplechecks.APIStatusError as e:
...
These methods return an [`APIResponse`](https://github.com/stainless-sdks/simplechecks-python/tree/main/src/simplechecks/_response.py) object.

The async client returns an [`AsyncAPIResponse`](https://github.com/stainless-sdks/simplechecks-python/tree/main/src/simplechecks/_response.py) with the same structure, the only difference being `await`able methods for reading the response content.
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/simplechecks-python/issues) with questions, bugs, or suggestions.
...
```py
import simplechecks
print(simplechecks.__version__)
```
