# File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.

from __future__ import annotations

import os
from typing import Any, cast

import pytest

from tests.utils import assert_matches_type
from simplechecks import SimpleChecks, AsyncSimpleChecks
from simplechecks.types import (
    MaintenanceWindow,
)
from simplechecks.pagination import SyncMaintenanceWindowsCursor, AsyncMaintenanceWindowsCursor

base_url = os.environ.get("TEST_API_BASE_URL", "http://127.0.0.1:4010")


class TestMaintenanceWindows:
    parametrize = pytest.mark.parametrize("client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    def test_method_create(self, client: SimpleChecks) -> None:
        maintenance_window = client.maintenance_windows.create(
            duration_ms=0,
            name="name",
            schedule_kind="one_time",
            start_unix_ms=0,
        )
        assert_matches_type(MaintenanceWindow, maintenance_window, path=["response"])

    @parametrize
    def test_method_create_with_all_params(self, client: SimpleChecks) -> None:
        maintenance_window = client.maintenance_windows.create(
            duration_ms=0,
            name="name",
            schedule_kind="one_time",
            start_unix_ms=0,
            check_ids=["182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e"],
            check_tags=["string"],
            repeat_ends_unix_ms=0,
            repeat_interval=0,
            repeat_unit="DAY",
            timezone="timezone",
        )
        assert_matches_type(MaintenanceWindow, maintenance_window, path=["response"])

    @parametrize
    def test_raw_response_create(self, client: SimpleChecks) -> None:
        response = client.maintenance_windows.with_raw_response.create(
            duration_ms=0,
            name="name",
            schedule_kind="one_time",
            start_unix_ms=0,
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        maintenance_window = response.parse()
        assert_matches_type(MaintenanceWindow, maintenance_window, path=["response"])

    @parametrize
    def test_streaming_response_create(self, client: SimpleChecks) -> None:
        with client.maintenance_windows.with_streaming_response.create(
            duration_ms=0,
            name="name",
            schedule_kind="one_time",
            start_unix_ms=0,
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            maintenance_window = response.parse()
            assert_matches_type(MaintenanceWindow, maintenance_window, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    def test_method_retrieve(self, client: SimpleChecks) -> None:
        maintenance_window = client.maintenance_windows.retrieve(
            "id",
        )
        assert_matches_type(MaintenanceWindow, maintenance_window, path=["response"])

    @parametrize
    def test_raw_response_retrieve(self, client: SimpleChecks) -> None:
        response = client.maintenance_windows.with_raw_response.retrieve(
            "id",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        maintenance_window = response.parse()
        assert_matches_type(MaintenanceWindow, maintenance_window, path=["response"])

    @parametrize
    def test_streaming_response_retrieve(self, client: SimpleChecks) -> None:
        with client.maintenance_windows.with_streaming_response.retrieve(
            "id",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            maintenance_window = response.parse()
            assert_matches_type(MaintenanceWindow, maintenance_window, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    def test_path_params_retrieve(self, client: SimpleChecks) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `id` but received ''"):
            client.maintenance_windows.with_raw_response.retrieve(
                "",
            )

    @parametrize
    def test_method_update(self, client: SimpleChecks) -> None:
        maintenance_window = client.maintenance_windows.update(
            id="id",
        )
        assert_matches_type(MaintenanceWindow, maintenance_window, path=["response"])

    @parametrize
    def test_method_update_with_all_params(self, client: SimpleChecks) -> None:
        maintenance_window = client.maintenance_windows.update(
            id="id",
            check_ids=["182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e"],
            check_tags=["string"],
            duration_ms=0,
            name="name",
            repeat_ends_unix_ms=0,
            repeat_interval=0,
            repeat_unit="DAY",
            schedule_kind="one_time",
            start_unix_ms=0,
            timezone="timezone",
        )
        assert_matches_type(MaintenanceWindow, maintenance_window, path=["response"])

    @parametrize
    def test_raw_response_update(self, client: SimpleChecks) -> None:
        response = client.maintenance_windows.with_raw_response.update(
            id="id",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        maintenance_window = response.parse()
        assert_matches_type(MaintenanceWindow, maintenance_window, path=["response"])

    @parametrize
    def test_streaming_response_update(self, client: SimpleChecks) -> None:
        with client.maintenance_windows.with_streaming_response.update(
            id="id",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            maintenance_window = response.parse()
            assert_matches_type(MaintenanceWindow, maintenance_window, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    def test_path_params_update(self, client: SimpleChecks) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `id` but received ''"):
            client.maintenance_windows.with_raw_response.update(
                id="",
            )

    @parametrize
    def test_method_list(self, client: SimpleChecks) -> None:
        maintenance_window = client.maintenance_windows.list()
        assert_matches_type(SyncMaintenanceWindowsCursor[MaintenanceWindow], maintenance_window, path=["response"])

    @parametrize
    def test_method_list_with_all_params(self, client: SimpleChecks) -> None:
        maintenance_window = client.maintenance_windows.list(
            cursor="cursor",
            limit=1,
        )
        assert_matches_type(SyncMaintenanceWindowsCursor[MaintenanceWindow], maintenance_window, path=["response"])

    @parametrize
    def test_raw_response_list(self, client: SimpleChecks) -> None:
        response = client.maintenance_windows.with_raw_response.list()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        maintenance_window = response.parse()
        assert_matches_type(SyncMaintenanceWindowsCursor[MaintenanceWindow], maintenance_window, path=["response"])

    @parametrize
    def test_streaming_response_list(self, client: SimpleChecks) -> None:
        with client.maintenance_windows.with_streaming_response.list() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            maintenance_window = response.parse()
            assert_matches_type(SyncMaintenanceWindowsCursor[MaintenanceWindow], maintenance_window, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    def test_method_delete(self, client: SimpleChecks) -> None:
        maintenance_window = client.maintenance_windows.delete(
            "id",
        )
        assert maintenance_window is None

    @parametrize
    def test_raw_response_delete(self, client: SimpleChecks) -> None:
        response = client.maintenance_windows.with_raw_response.delete(
            "id",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        maintenance_window = response.parse()
        assert maintenance_window is None

    @parametrize
    def test_streaming_response_delete(self, client: SimpleChecks) -> None:
        with client.maintenance_windows.with_streaming_response.delete(
            "id",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            maintenance_window = response.parse()
            assert maintenance_window is None

        assert cast(Any, response.is_closed) is True

    @parametrize
    def test_path_params_delete(self, client: SimpleChecks) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `id` but received ''"):
            client.maintenance_windows.with_raw_response.delete(
                "",
            )


class TestAsyncMaintenanceWindows:
    parametrize = pytest.mark.parametrize(
        "async_client", [False, True, {"http_client": "aiohttp"}], indirect=True, ids=["loose", "strict", "aiohttp"]
    )

    @parametrize
    async def test_method_create(self, async_client: AsyncSimpleChecks) -> None:
        maintenance_window = await async_client.maintenance_windows.create(
            duration_ms=0,
            name="name",
            schedule_kind="one_time",
            start_unix_ms=0,
        )
        assert_matches_type(MaintenanceWindow, maintenance_window, path=["response"])

    @parametrize
    async def test_method_create_with_all_params(self, async_client: AsyncSimpleChecks) -> None:
        maintenance_window = await async_client.maintenance_windows.create(
            duration_ms=0,
            name="name",
            schedule_kind="one_time",
            start_unix_ms=0,
            check_ids=["182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e"],
            check_tags=["string"],
            repeat_ends_unix_ms=0,
            repeat_interval=0,
            repeat_unit="DAY",
            timezone="timezone",
        )
        assert_matches_type(MaintenanceWindow, maintenance_window, path=["response"])

    @parametrize
    async def test_raw_response_create(self, async_client: AsyncSimpleChecks) -> None:
        response = await async_client.maintenance_windows.with_raw_response.create(
            duration_ms=0,
            name="name",
            schedule_kind="one_time",
            start_unix_ms=0,
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        maintenance_window = await response.parse()
        assert_matches_type(MaintenanceWindow, maintenance_window, path=["response"])

    @parametrize
    async def test_streaming_response_create(self, async_client: AsyncSimpleChecks) -> None:
        async with async_client.maintenance_windows.with_streaming_response.create(
            duration_ms=0,
            name="name",
            schedule_kind="one_time",
            start_unix_ms=0,
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            maintenance_window = await response.parse()
            assert_matches_type(MaintenanceWindow, maintenance_window, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    async def test_method_retrieve(self, async_client: AsyncSimpleChecks) -> None:
        maintenance_window = await async_client.maintenance_windows.retrieve(
            "id",
        )
        assert_matches_type(MaintenanceWindow, maintenance_window, path=["response"])

    @parametrize
    async def test_raw_response_retrieve(self, async_client: AsyncSimpleChecks) -> None:
        response = await async_client.maintenance_windows.with_raw_response.retrieve(
            "id",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        maintenance_window = await response.parse()
        assert_matches_type(MaintenanceWindow, maintenance_window, path=["response"])

    @parametrize
    async def test_streaming_response_retrieve(self, async_client: AsyncSimpleChecks) -> None:
        async with async_client.maintenance_windows.with_streaming_response.retrieve(
            "id",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            maintenance_window = await response.parse()
            assert_matches_type(MaintenanceWindow, maintenance_window, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    async def test_path_params_retrieve(self, async_client: AsyncSimpleChecks) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `id` but received ''"):
            await async_client.maintenance_windows.with_raw_response.retrieve(
                "",
            )

    @parametrize
    async def test_method_update(self, async_client: AsyncSimpleChecks) -> None:
        maintenance_window = await async_client.maintenance_windows.update(
            id="id",
        )
        assert_matches_type(MaintenanceWindow, maintenance_window, path=["response"])

    @parametrize
    async def test_method_update_with_all_params(self, async_client: AsyncSimpleChecks) -> None:
        maintenance_window = await async_client.maintenance_windows.update(
            id="id",
            check_ids=["182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e"],
            check_tags=["string"],
            duration_ms=0,
            name="name",
            repeat_ends_unix_ms=0,
            repeat_interval=0,
            repeat_unit="DAY",
            schedule_kind="one_time",
            start_unix_ms=0,
            timezone="timezone",
        )
        assert_matches_type(MaintenanceWindow, maintenance_window, path=["response"])

    @parametrize
    async def test_raw_response_update(self, async_client: AsyncSimpleChecks) -> None:
        response = await async_client.maintenance_windows.with_raw_response.update(
            id="id",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        maintenance_window = await response.parse()
        assert_matches_type(MaintenanceWindow, maintenance_window, path=["response"])

    @parametrize
    async def test_streaming_response_update(self, async_client: AsyncSimpleChecks) -> None:
        async with async_client.maintenance_windows.with_streaming_response.update(
            id="id",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            maintenance_window = await response.parse()
            assert_matches_type(MaintenanceWindow, maintenance_window, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    async def test_path_params_update(self, async_client: AsyncSimpleChecks) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `id` but received ''"):
            await async_client.maintenance_windows.with_raw_response.update(
                id="",
            )

    @parametrize
    async def test_method_list(self, async_client: AsyncSimpleChecks) -> None:
        maintenance_window = await async_client.maintenance_windows.list()
        assert_matches_type(AsyncMaintenanceWindowsCursor[MaintenanceWindow], maintenance_window, path=["response"])

    @parametrize
    async def test_method_list_with_all_params(self, async_client: AsyncSimpleChecks) -> None:
        maintenance_window = await async_client.maintenance_windows.list(
            cursor="cursor",
            limit=1,
        )
        assert_matches_type(AsyncMaintenanceWindowsCursor[MaintenanceWindow], maintenance_window, path=["response"])

    @parametrize
    async def test_raw_response_list(self, async_client: AsyncSimpleChecks) -> None:
        response = await async_client.maintenance_windows.with_raw_response.list()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        maintenance_window = await response.parse()
        assert_matches_type(AsyncMaintenanceWindowsCursor[MaintenanceWindow], maintenance_window, path=["response"])

    @parametrize
    async def test_streaming_response_list(self, async_client: AsyncSimpleChecks) -> None:
        async with async_client.maintenance_windows.with_streaming_response.list() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            maintenance_window = await response.parse()
            assert_matches_type(AsyncMaintenanceWindowsCursor[MaintenanceWindow], maintenance_window, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    async def test_method_delete(self, async_client: AsyncSimpleChecks) -> None:
        maintenance_window = await async_client.maintenance_windows.delete(
            "id",
        )
        assert maintenance_window is None

    @parametrize
    async def test_raw_response_delete(self, async_client: AsyncSimpleChecks) -> None:
        response = await async_client.maintenance_windows.with_raw_response.delete(
            "id",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        maintenance_window = await response.parse()
        assert maintenance_window is None

    @parametrize
    async def test_streaming_response_delete(self, async_client: AsyncSimpleChecks) -> None:
        async with async_client.maintenance_windows.with_streaming_response.delete(
            "id",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            maintenance_window = await response.parse()
            assert maintenance_window is None

        assert cast(Any, response.is_closed) is True

    @parametrize
    async def test_path_params_delete(self, async_client: AsyncSimpleChecks) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `id` but received ''"):
            await async_client.maintenance_windows.with_raw_response.delete(
                "",
            )
