# File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.

from __future__ import annotations

import os
from typing import Any, cast

import pytest

from tests.utils import assert_matches_type
from simplechecks import SimpleChecks, AsyncSimpleChecks
from simplechecks.types import (
    AlertSubscription,
)
from simplechecks.pagination import SyncAlertSubscriptionsCursor, AsyncAlertSubscriptionsCursor

base_url = os.environ.get("TEST_API_BASE_URL", "http://127.0.0.1:4010")


class TestAlertSubscriptions:
    parametrize = pytest.mark.parametrize("client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    def test_method_create(self, client: SimpleChecks) -> None:
        alert_subscription = client.alert_subscriptions.create(
            channel_id="channel_id",
            check_id="182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
        )
        assert_matches_type(AlertSubscription, alert_subscription, path=["response"])

    @parametrize
    def test_method_create_with_all_params(self, client: SimpleChecks) -> None:
        alert_subscription = client.alert_subscriptions.create(
            channel_id="channel_id",
            check_id="182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
            notify_on_failure=True,
            notify_on_recovery=True,
        )
        assert_matches_type(AlertSubscription, alert_subscription, path=["response"])

    @parametrize
    def test_raw_response_create(self, client: SimpleChecks) -> None:
        response = client.alert_subscriptions.with_raw_response.create(
            channel_id="channel_id",
            check_id="182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        alert_subscription = response.parse()
        assert_matches_type(AlertSubscription, alert_subscription, path=["response"])

    @parametrize
    def test_streaming_response_create(self, client: SimpleChecks) -> None:
        with client.alert_subscriptions.with_streaming_response.create(
            channel_id="channel_id",
            check_id="182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            alert_subscription = response.parse()
            assert_matches_type(AlertSubscription, alert_subscription, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    def test_method_retrieve(self, client: SimpleChecks) -> None:
        alert_subscription = client.alert_subscriptions.retrieve(
            "id",
        )
        assert_matches_type(AlertSubscription, alert_subscription, path=["response"])

    @parametrize
    def test_raw_response_retrieve(self, client: SimpleChecks) -> None:
        response = client.alert_subscriptions.with_raw_response.retrieve(
            "id",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        alert_subscription = response.parse()
        assert_matches_type(AlertSubscription, alert_subscription, path=["response"])

    @parametrize
    def test_streaming_response_retrieve(self, client: SimpleChecks) -> None:
        with client.alert_subscriptions.with_streaming_response.retrieve(
            "id",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            alert_subscription = response.parse()
            assert_matches_type(AlertSubscription, alert_subscription, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    def test_path_params_retrieve(self, client: SimpleChecks) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `id` but received ''"):
            client.alert_subscriptions.with_raw_response.retrieve(
                "",
            )

    @parametrize
    def test_method_update(self, client: SimpleChecks) -> None:
        alert_subscription = client.alert_subscriptions.update(
            id="id",
        )
        assert_matches_type(AlertSubscription, alert_subscription, path=["response"])

    @parametrize
    def test_method_update_with_all_params(self, client: SimpleChecks) -> None:
        alert_subscription = client.alert_subscriptions.update(
            id="id",
            notify_on_failure=True,
            notify_on_recovery=True,
        )
        assert_matches_type(AlertSubscription, alert_subscription, path=["response"])

    @parametrize
    def test_raw_response_update(self, client: SimpleChecks) -> None:
        response = client.alert_subscriptions.with_raw_response.update(
            id="id",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        alert_subscription = response.parse()
        assert_matches_type(AlertSubscription, alert_subscription, path=["response"])

    @parametrize
    def test_streaming_response_update(self, client: SimpleChecks) -> None:
        with client.alert_subscriptions.with_streaming_response.update(
            id="id",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            alert_subscription = response.parse()
            assert_matches_type(AlertSubscription, alert_subscription, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    def test_path_params_update(self, client: SimpleChecks) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `id` but received ''"):
            client.alert_subscriptions.with_raw_response.update(
                id="",
            )

    @parametrize
    def test_method_list(self, client: SimpleChecks) -> None:
        alert_subscription = client.alert_subscriptions.list()
        assert_matches_type(SyncAlertSubscriptionsCursor[AlertSubscription], alert_subscription, path=["response"])

    @parametrize
    def test_method_list_with_all_params(self, client: SimpleChecks) -> None:
        alert_subscription = client.alert_subscriptions.list(
            channel_id="channel_id",
            check_id="182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
            cursor="cursor",
            limit=1,
        )
        assert_matches_type(SyncAlertSubscriptionsCursor[AlertSubscription], alert_subscription, path=["response"])

    @parametrize
    def test_raw_response_list(self, client: SimpleChecks) -> None:
        response = client.alert_subscriptions.with_raw_response.list()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        alert_subscription = response.parse()
        assert_matches_type(SyncAlertSubscriptionsCursor[AlertSubscription], alert_subscription, path=["response"])

    @parametrize
    def test_streaming_response_list(self, client: SimpleChecks) -> None:
        with client.alert_subscriptions.with_streaming_response.list() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            alert_subscription = response.parse()
            assert_matches_type(SyncAlertSubscriptionsCursor[AlertSubscription], alert_subscription, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    def test_method_delete(self, client: SimpleChecks) -> None:
        alert_subscription = client.alert_subscriptions.delete(
            "id",
        )
        assert alert_subscription is None

    @parametrize
    def test_raw_response_delete(self, client: SimpleChecks) -> None:
        response = client.alert_subscriptions.with_raw_response.delete(
            "id",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        alert_subscription = response.parse()
        assert alert_subscription is None

    @parametrize
    def test_streaming_response_delete(self, client: SimpleChecks) -> None:
        with client.alert_subscriptions.with_streaming_response.delete(
            "id",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            alert_subscription = response.parse()
            assert alert_subscription is None

        assert cast(Any, response.is_closed) is True

    @parametrize
    def test_path_params_delete(self, client: SimpleChecks) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `id` but received ''"):
            client.alert_subscriptions.with_raw_response.delete(
                "",
            )


class TestAsyncAlertSubscriptions:
    parametrize = pytest.mark.parametrize(
        "async_client", [False, True, {"http_client": "aiohttp"}], indirect=True, ids=["loose", "strict", "aiohttp"]
    )

    @parametrize
    async def test_method_create(self, async_client: AsyncSimpleChecks) -> None:
        alert_subscription = await async_client.alert_subscriptions.create(
            channel_id="channel_id",
            check_id="182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
        )
        assert_matches_type(AlertSubscription, alert_subscription, path=["response"])

    @parametrize
    async def test_method_create_with_all_params(self, async_client: AsyncSimpleChecks) -> None:
        alert_subscription = await async_client.alert_subscriptions.create(
            channel_id="channel_id",
            check_id="182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
            notify_on_failure=True,
            notify_on_recovery=True,
        )
        assert_matches_type(AlertSubscription, alert_subscription, path=["response"])

    @parametrize
    async def test_raw_response_create(self, async_client: AsyncSimpleChecks) -> None:
        response = await async_client.alert_subscriptions.with_raw_response.create(
            channel_id="channel_id",
            check_id="182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        alert_subscription = await response.parse()
        assert_matches_type(AlertSubscription, alert_subscription, path=["response"])

    @parametrize
    async def test_streaming_response_create(self, async_client: AsyncSimpleChecks) -> None:
        async with async_client.alert_subscriptions.with_streaming_response.create(
            channel_id="channel_id",
            check_id="182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            alert_subscription = await response.parse()
            assert_matches_type(AlertSubscription, alert_subscription, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    async def test_method_retrieve(self, async_client: AsyncSimpleChecks) -> None:
        alert_subscription = await async_client.alert_subscriptions.retrieve(
            "id",
        )
        assert_matches_type(AlertSubscription, alert_subscription, path=["response"])

    @parametrize
    async def test_raw_response_retrieve(self, async_client: AsyncSimpleChecks) -> None:
        response = await async_client.alert_subscriptions.with_raw_response.retrieve(
            "id",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        alert_subscription = await response.parse()
        assert_matches_type(AlertSubscription, alert_subscription, path=["response"])

    @parametrize
    async def test_streaming_response_retrieve(self, async_client: AsyncSimpleChecks) -> None:
        async with async_client.alert_subscriptions.with_streaming_response.retrieve(
            "id",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            alert_subscription = await response.parse()
            assert_matches_type(AlertSubscription, alert_subscription, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    async def test_path_params_retrieve(self, async_client: AsyncSimpleChecks) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `id` but received ''"):
            await async_client.alert_subscriptions.with_raw_response.retrieve(
                "",
            )

    @parametrize
    async def test_method_update(self, async_client: AsyncSimpleChecks) -> None:
        alert_subscription = await async_client.alert_subscriptions.update(
            id="id",
        )
        assert_matches_type(AlertSubscription, alert_subscription, path=["response"])

    @parametrize
    async def test_method_update_with_all_params(self, async_client: AsyncSimpleChecks) -> None:
        alert_subscription = await async_client.alert_subscriptions.update(
            id="id",
            notify_on_failure=True,
            notify_on_recovery=True,
        )
        assert_matches_type(AlertSubscription, alert_subscription, path=["response"])

    @parametrize
    async def test_raw_response_update(self, async_client: AsyncSimpleChecks) -> None:
        response = await async_client.alert_subscriptions.with_raw_response.update(
            id="id",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        alert_subscription = await response.parse()
        assert_matches_type(AlertSubscription, alert_subscription, path=["response"])

    @parametrize
    async def test_streaming_response_update(self, async_client: AsyncSimpleChecks) -> None:
        async with async_client.alert_subscriptions.with_streaming_response.update(
            id="id",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            alert_subscription = await response.parse()
            assert_matches_type(AlertSubscription, alert_subscription, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    async def test_path_params_update(self, async_client: AsyncSimpleChecks) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `id` but received ''"):
            await async_client.alert_subscriptions.with_raw_response.update(
                id="",
            )

    @parametrize
    async def test_method_list(self, async_client: AsyncSimpleChecks) -> None:
        alert_subscription = await async_client.alert_subscriptions.list()
        assert_matches_type(AsyncAlertSubscriptionsCursor[AlertSubscription], alert_subscription, path=["response"])

    @parametrize
    async def test_method_list_with_all_params(self, async_client: AsyncSimpleChecks) -> None:
        alert_subscription = await async_client.alert_subscriptions.list(
            channel_id="channel_id",
            check_id="182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
            cursor="cursor",
            limit=1,
        )
        assert_matches_type(AsyncAlertSubscriptionsCursor[AlertSubscription], alert_subscription, path=["response"])

    @parametrize
    async def test_raw_response_list(self, async_client: AsyncSimpleChecks) -> None:
        response = await async_client.alert_subscriptions.with_raw_response.list()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        alert_subscription = await response.parse()
        assert_matches_type(AsyncAlertSubscriptionsCursor[AlertSubscription], alert_subscription, path=["response"])

    @parametrize
    async def test_streaming_response_list(self, async_client: AsyncSimpleChecks) -> None:
        async with async_client.alert_subscriptions.with_streaming_response.list() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            alert_subscription = await response.parse()
            assert_matches_type(AsyncAlertSubscriptionsCursor[AlertSubscription], alert_subscription, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    async def test_method_delete(self, async_client: AsyncSimpleChecks) -> None:
        alert_subscription = await async_client.alert_subscriptions.delete(
            "id",
        )
        assert alert_subscription is None

    @parametrize
    async def test_raw_response_delete(self, async_client: AsyncSimpleChecks) -> None:
        response = await async_client.alert_subscriptions.with_raw_response.delete(
            "id",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        alert_subscription = await response.parse()
        assert alert_subscription is None

    @parametrize
    async def test_streaming_response_delete(self, async_client: AsyncSimpleChecks) -> None:
        async with async_client.alert_subscriptions.with_streaming_response.delete(
            "id",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            alert_subscription = await response.parse()
            assert alert_subscription is None

        assert cast(Any, response.is_closed) is True

    @parametrize
    async def test_path_params_delete(self, async_client: AsyncSimpleChecks) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `id` but received ''"):
            await async_client.alert_subscriptions.with_raw_response.delete(
                "",
            )
